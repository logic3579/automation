...
This repository contains two independent automation stacks. `ansible/` is the primary path for provisioning and service rollout: `playbooks/` holds entry points such as `init.yml`, `nginx.yml`, and `redis.yml`; `roles/` uses the standard role layout; `inventories/` contains `hosts`, `init.hosts`, and `group_vars/` split by region. `saltproject/` contains Salt states in `base/` and pillar data in `pillar/`. Keep changes inside the matching role or state tree instead of adding root-level scripts.
...
- `ansible-playbook playbooks/nginx.yml --limit nginx_east` applies a service playbook to one inventory group.
- `ansible-playbook -i inventories/init.hosts playbooks/init.yml --limit init_east --vault-id pwd.vault` bootstraps new hosts.
...
## Architecture Notes
Inventory is organized by region, with service groups such as `nginx_east` or `redis_east`. Playbooks typically target `hosts: all` and narrow execution with `--limit`. In `playbooks/init.yml`, role order matters: bootstrap runs from `hostname` through `sshd`, and `sshd` is intentionally last because it changes the port and disables password login. Keep `sshd_port` and firewall settings aligned when editing bootstrap behavior.
